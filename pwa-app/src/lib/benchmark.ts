...
export async function measureOperation<T>(
  ...
  const hasMemory =
    (performance as any).memory !== undefined && options.measureMemory;

  const memoryBefore = hasMemory
  ...
  return {
    ...
    memoryDeltaMB:
      hasMemory && memoryBefore !== null && memoryAfter !== null
...
export async function runMultiple<T>(
  ...
  for (let i = 0; i < runs; i++) {
    results.push(
      await measureOperation(`${label} #${i + 1}`, operation, options),
    );
  }
  return results;
}
